...
export const buildExperienceShareTargetPath = (query = {}) => {
  ...
  return `/website${queryString ? `?${queryString}` : ''}`;
};

export const buildExperienceShareCopy = (lang = 'en', city = '') => {
  const isFrench = lang === 'fr';
  const cityLabel = safeText(city, 'Tangier');

  return {
    pageTitle: isFrench ? 'Choisissez votre expérience | SaharaX' : 'Choose your experience | SaharaX',
    ogTitle: isFrench ? 'Choisissez votre expérience SaharaX' : 'Choose your SaharaX experience',
    ogDescription: isFrench
      ? `Location, tours et marketplace à ${cityLabel}. Ouvrez l'expérience SaharaX dans la bonne langue.`
      : `Rent, tours, and marketplace in ${cityLabel}. Open the SaharaX experience in the right language.`,
    eyebrow: isFrench ? 'EXPÉRIENCE SAHARAX' : 'SAHARAX EXPERIENCE',
    heading: isFrench ? 'Choisissez votre expérience' : 'Choose your experience',
    subtitle: isFrench
      ? `Location, tours et marketplace à ${cityLabel}.`
      : `Rent, tours, and marketplace in ${cityLabel}.`,
  };
};

export const renderExperienceShareHtml = (lang, copy, shareUrl, imageUrl, targetUrl) => `<!doctype html>
...
export default async function handler(req, res) {
  try {
    ...
    const copy = buildExperienceShareCopy(lang, city);
    const targetPath = buildExperienceShareTargetPath(passthroughQuery);
    ...
    const shareUrl = `${origin}/share/experience${shareQuery.toString() ? `?${shareQuery.toString()}` : ''}`;
    ...
    return sendResponse(
      ...
      renderExperienceShareHtml(lang, copy, shareUrl, imageUrl, targetUrl),
    );
  } catch (error) {
    return sendResponse(res, 500, 'text/plain; charset=utf-8', error?.message || 'Unable to render experience share page');
  }
}
